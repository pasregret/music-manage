...
export default {
  ...
  methods: {
    ...
    saveEdit() {
      ...
      _this
        .$axios({
          url: `${_this.$store.state.HOST}/api/updateSongListMsgs`,
          method: "post",
          dataType: "json",
          data: {
            id: _this.form.id,
    ...
    addsongList() {
      ...
      _this
        .$axios({
          url: `${_this.$store.state.HOST}/api/addSongList`,
          method: "post",
          dataType: "josn",
          data: {
            title: _this.registerForm.title,
            pic: "/img/songListPic/123.jpg",
            introduction: _this.registerForm.introduction,
            style: _this.registerForm.style
          }
        })
        .then(res => {
          if (res.data.code === 1) {
    ...
    deleteRow() {
      let _this = this;
      _this.$axios
        .get(`${_this.$store.state.HOST}/api/deleteSongLists/${_this.idx}`)
        .then(res => {
          if (res.data) {
            _this.getData();
            _this.$notify({
